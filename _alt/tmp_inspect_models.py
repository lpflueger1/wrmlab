...
try:
    ...
    for station, dfQ, dfC in [('Diepoldsau', qD_log, sscD_log), ('Gisingen', qG_log, sscG_log)]:
        for var, df, col in [('Q', dfQ, 'q_m3s'), ('C', dfC, dfC.columns[0])]:
            key = f'{var}_{station}'
            z, m_mean, m_std = hf.seasonal_standardise(df[col].dropna())
            det, res = hf.test_and_detrend(z.to_frame(name=col), col)
    ...
    for k,m in fitted.items():
        print(k, 'model_type', type(m), 'arparams', getattr(m, 'arparams', None), 'maparams', getattr(m, 'maparams', None))
except Exception:
    traceback.print_exc()
